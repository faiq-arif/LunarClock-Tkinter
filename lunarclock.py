...
def time():
    ...
    loc.lat=latDec
    loc.lon=longDec
    loc.date = datetime.now(timezone)
    sun = ephem.Sun()
    sunset=ephem.localtime(loc.next_setting(sun))
    sunset_hour=int(sunset.strftime("%H"))
    sunset_min=int(sunset.strftime("%M"))
    sunset_sec=int(sunset.strftime("%S"))
    mid_hour = 23 - sunset_hour
    mid_min = 59 - sunset_min
    mid_sec = 59 - sunset_sec
    add_sunset=datetime.now(timezone) + timedelta(hours=mid_hour, minutes=mid_min, seconds=mid_sec)
    #add_sunset=datetime(2020, 8, 29, hour=17, minute=36, second=12) + timedelta(hours=mid_hour, minutes=mid_min, seconds=mid_sec)
    gYear = int(add_sunset.strftime("%Y"))
    gMonth = int(add_sunset.strftime("%m"))
    gDay = int(add_sunset.strftime("%d"))
    hijri = convert.Gregorian(gYear, gMonth, gDay).to_hijri()
    hd = hijri.day
    hm = hijri.month_name()
    hy = hijri.year
    hijriDate = hd,hm,hy
...
labelPhase = Label(frameDisp, font = ('calibri', 37, 'bold'),
			background = 'black',
			foreground = 'white', text="Moon Phase")
labelAlt = Label(frameDisp, font = ('calibri', 37, 'bold'),
			background = 'black',
			foreground = 'white', text="Altitude")
labelAz = Label(frameDisp, font = ('calibri', 37, 'bold'),
			background = 'black',
			foreground = 'white', text="Azimuth")
labelEl = Label(frameDisp, font = ('calibri', 37, 'bold'),
			background = 'black',
			foreground = 'white', text="Elongation")
dataPhase = Label(frameDisp, font = ('calibri', 37),
			background = 'black')
dataAlt = Label(frameDisp, font = ('calibri', 37),
			background = 'black')
dataAz = Label(frameDisp, font = ('calibri', 37),
			background = 'black')
dataEl = Label(frameDisp, font = ('calibri', 37),
			background = 'black')
credit = Label(frameDisp, font = ('calibri', 10),
			background = 'black', justify=RIGHT,
			foreground = 'white', text="Club Astronomi Santri Assalaam 2018/2019\nProgrammer: Faiq M. Arif (faiq.m.arif) | Hardware: Arief R. P. | Moral Support: Amir Abdullah")  
...
labelPhase.grid(row=3, column=8, rowspan=1, columnspan=4, sticky=E)
dataPhase.grid(row=3, column=12, rowspan=1, columnspan=4, sticky=W, padx=5)
labelAlt.grid(row=4, column=8, rowspan=1, columnspan=4, sticky=E)
dataAlt.grid(row=4, column=12, rowspan=1, columnspan=4, sticky=W, padx=5)
labelAz.grid(row=5, column=8, rowspan=1, columnspan=4, sticky=E)
dataAz.grid(row=5, column=12, rowspan=1, columnspan=4, sticky=W, padx=5)
labelEl.grid(row=6, column=8, rowspan=1, columnspan=4, sticky=E)
dataEl.grid(row=6, column=12, rowspan=1, columnspan=4, sticky=W, padx=5)
credit.grid(row=7, columnspan=16, sticky=SE, padx=5)
time()
